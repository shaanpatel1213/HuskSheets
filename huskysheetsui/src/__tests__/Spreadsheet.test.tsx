...
describe('Spreadsheet Component', () => {
  ...
  test('renders initial 100 rows and 25 columns', () => {
    ...
    // Check that there are 100 rows + 1 header row
    const rows = screen.getAllByRole('row');
    expect(rows).toHaveLength(initialRows + 1);

    // Check that there are 25 columns + 1 header column
    const cols = screen.getAllByRole('columnheader');
    expect(cols).toHaveLength(initialCols + 1);
  });
  ...
  test('adds a row when "Add Row" button is clicked', () => {
    ...
    const rows = screen.getAllByRole('row');
    expect(rows).toHaveLength(initialRows + 2); // 101 rows + 1 header row
  });
  ...
  test('removes a row when "Remove Row" button is clicked', () => {
    ...
    const rows = screen.getAllByRole('row');
    expect(rows).toHaveLength(initialRows); // 99 rows + 1 header row
  });
